...
test("reader shows the latest snapshot without hour filters", () => {
  ...
  const hnRefresh = fs.readFileSync("scripts/add-hacker-news-top10.js", "utf8");
  const readerRefresh = fs.readFileSync("scripts/refresh-reader-news.js", "utf8");
  const aggregatedRefresh = fs.readFileSync("scripts/refresh-reader-aggregated.js", "utf8");
  const youtubeRefresh = fs.readFileSync("scripts/refresh-youtube-top10-best-effort.js", "utf8");
  const finalVerification = fs.readFileSync("scripts/verify-reader-refresh.js", "utf8");
  ...
  assert.match(html, /data-category="favorites"[^>]*>收藏</);
  assert.ok(html.indexOf('data-category="favorites"') < html.indexOf('data-category="tech"'));
  ...
  assert.match(hnScript, /FAVORITES_KEY = 'globalnews-reader-favorites-v1'/);
  assert.match(hnScript, /localStorage\.setItem\(FAVORITES_KEY/);
  assert.match(hnScript, /function addFavorite/);
  assert.match(hnScript, /function removeFavorite/);
  assert.match(hnScript, /activeCategory === 'favorites'/);
  ...
  assert.match(youtubeRefresh, /addChineseTranslations\(items\.map/);
  ...
  assert.match(script, /item\.titleZh && item\.titleZh !== item\.title/);
